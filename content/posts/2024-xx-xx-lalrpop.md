...
Я уже много лет работаю разработчиком и в какой-то момент давным давно я захотел написать свой язык программирования.
Изначально описал только синтаскис и думал заставить его работать очень сложно. 
Со временем синткис менялся, из-за моих привычек в программировании,
всё больше начал склоняться к функциональному стилю и применять его даже к таким языкам как Python, Rust, Go.
Относительно недавно мне захотелось заставить синтаксис что-то делать и так как я стал больше использовать Rust, решил неплохо бы сделать на нём плюс один язык.
Первым делом я понимал, что надо распарсить код в обьекты, которые удобно обрабатывать программно.
На расте оказался подходящий инструмент для этой задачи, и это [LALRPOP](https://github.com/lalrpop/lalrpop).
...
В посте опишу основные возможности и свой опыт, также существует [первоисточник](https://lalrpop.github.io/lalrpop/), где есть примеры и подробное описание.
...
До того, как я написал свою "идеальную грамматику" и "язык мечты", я просто решил взять готовый инструмент на Rust-е с примерами и повторять нужные конструкции. В качестве примера взял грамматику калькулятора. Когда начало что-то простое получаться, я подумал: "это проще, чем мне казалось", но в некоторых кейсах возникали ошибки парсинга и мне приходилось терять много времени просто экспереминтируя с синтаксисом, потому что изначальная идея казалась не реализуемой. 
Изначально, я просто добивался рабочего примера, добавлял костыли, например, всё разделял ";", даже блоки конструкций типа struct или function. Код выглядел вот так:
